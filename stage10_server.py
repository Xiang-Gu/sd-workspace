...
DATA_ROOT = os.path.join(BASE_DIRECTORY, ".stage10_data")
...
def get_resource_profile(profile_name: str) -> dict:
    ...
    if profile is None:
        raise ValueError(
            "resource_profile must be one of: "
            + ", ".join(sorted(RESOURCE_PROFILES))
        )
    return profile
...
def get_container_name(workspace_id: str) -> str:
    return f"stage10-workspace-{workspace_id}"
...
def active_workspaces() -> list[Workspace]:
    return [
        workspace
        for workspace in WORKSPACES.values()
        if workspace.status == "ready" and workspace.runtime is not None and not workspace.runtime.closed
    ]


def get_active_resource_usage() -> dict:
    running = active_workspaces()
    return {
        "workspace_count": len(running),
        "cpus": sum(workspace.cpu_limit for workspace in running),
        "memory_mb": sum(workspace.memory_limit_mb for workspace in running),
    }


def ensure_capacity_for_workspace(candidate: Workspace) -> None:
    usage = get_active_resource_usage()
    ...
    if next_workspace_count > MAX_ACTIVE_WORKSPACES:
        raise ValueError(
            f"capacity exceeded: only {MAX_ACTIVE_WORKSPACES} ready workspaces are allowed at once"
        )
    if next_cpus > MAX_ACTIVE_CPUS:
        raise ValueError(
            f"capacity exceeded: requested CPUs would raise active usage to {next_cpus:.1f}, "
            f"above limit {MAX_ACTIVE_CPUS:.1f}"
        )
    if next_memory_mb > MAX_ACTIVE_MEMORY_MB:
        raise ValueError(
            f"capacity exceeded: requested memory would raise active usage to {next_memory_mb} MB, "
            f"above limit {MAX_ACTIVE_MEMORY_MB} MB"
        )
...
def create_shell_runtime(
    ...
    process = subprocess.Popen(
        [
            ...
            "-i",
            "--cpus",
            str(workspace.cpu_limit),
            "--memory",
            f"{workspace.memory_limit_mb}m",
            "-v",
...
def provision_workspace(workspace: Workspace, lifecycle_status: str) -> None:
    ensure_capacity_for_workspace(workspace)
...
async def create_workspace_handler(request: web.Request) -> web.Response:
    ...
    print(f"resource_profile: {workspace.resource_profile}")
    print(f"cpu_limit: {workspace.cpu_limit}")
    print(f"memory_limit_mb: {workspace.memory_limit_mb}")
    print("=" * 80)
    ...
    return web.json_response(
        {
            ...
            "active_usage": get_active_resource_usage(),
            "capacity_limits": {
...
def main() -> None:
    ...
    print("Resource-limited workspace demo server")
    ...
    print(f"Resource profiles: {json.dumps(RESOURCE_PROFILES)}")
    print(
        "Capacity limits: "
        f"{MAX_ACTIVE_WORKSPACES} ready workspaces, "
