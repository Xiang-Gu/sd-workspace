...
def resolve_workspace_path(workspace_root: str, requested_path: str | None) -> tuple[str, str]:
    relative_path = requested_path or "."
    target_path = os.path.abspath(os.path.join(workspace_root, relative_path))
    if target_path != workspace_root and not target_path.startswith(f"{workspace_root}{os.sep}"):
        raise ValueError("path must stay inside the workspace root")
    normalized_relative = os.path.relpath(target_path, workspace_root)
...
class Workspace:
    ...
    def shell_working_directory(self) -> str:
        return self.repo_path or self.working_directory
    ...
    def mark_ready(self, runtime: ShellRuntime) -> None:
        ...
        self.status = "ready"
        if self.repo_url and self.bootstrap_status != "failed":
            self.bootstrap_status = "checked_out"
...
def cleanup_repo_path(repo_path: str) -> None:
    if os.path.isdir(repo_path):
        shutil.rmtree(repo_path)


def bootstrap_repo(workspace: Workspace) -> None:
    ...
    repo_path = os.path.join(workspace.working_directory, "repo")
    cleanup_repo_path(repo_path)

    clone_result = run_command(["git", "clone", workspace.repo_url, repo_path])
    ...
    if workspace.commit:
        checkout_result = run_command(["git", "-C", repo_path, "checkout", workspace.commit])
        ...
        checkout_result = run_command(["git", "-C", repo_path, "checkout", workspace.branch])
        if checkout_result.returncode != 0:
            cleanup_repo_path(repo_path)
            raise RuntimeError(
                checkout_result.stderr.strip()
    ...
    rev_parse_result = run_command(["git", "-C", repo_path, "rev-parse", "HEAD"])
    if rev_parse_result.returncode == 0:
        workspace.commit = rev_parse_result.stdout.strip()
...
def create_workspace(
    ...
    if repo_url:
        bootstrap_repo(workspace)

    runtime = create_shell_runtime(loop, workspace.shell_working_directory())
...
def list_directory_entries(target_path: str, workspace_root: str) -> list[dict]:
    entries = []
    for entry in sorted(
        ...
        relative_path = os.path.relpath(entry_path, workspace_root)
...
async def create_workspace_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        failed_workspace = Workspace(
            ...
            owner_id=owner_id.strip(),
            name=name.strip(),
            working_directory=os.path.abspath(requested_cwd),
            shell_path=DEFAULT_SHELL,
            created_at=utc_now_iso(),
    ...
    print(f"shell cwd: {workspace.shell_working_directory()}")
...
async def list_files_handler(request: web.Request) -> web.Response:
    ...
    try:
        target_path, normalized_relative = resolve_workspace_path(
...
def main() -> None:
    ...
    print("Repo-aware workspace demo server")
