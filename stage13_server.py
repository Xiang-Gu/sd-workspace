...
CONTROL_PLANE_DB_PATH = os.path.join(DATA_ROOT, "control_plane.db")
...
def worker_from_row(row: sqlite3.Row) -> WorkerRecord:
    return WorkerRecord(
        ...
        active_usage=json.loads(row["active_usage_json"]),
        capacity_limits=json.loads(row["capacity_limits_json"]),
        last_error=row["last_error"],
    )


def open_control_plane_db() -> sqlite3.Connection:
    os.makedirs(DATA_ROOT, exist_ok=True)
    db = sqlite3.connect(CONTROL_PLANE_DB_PATH)
    ...
    db.executescript(
        ...
        CREATE INDEX IF NOT EXISTS idx_workspaces_owner_name
        ON workspaces(owner_id, name, created_at);
        ...
        CREATE TABLE IF NOT EXISTS workers (
            ...
            last_seen_at TEXT,
            active_usage_json TEXT NOT NULL,
            capacity_limits_json TEXT NOT NULL,
            last_error TEXT
        );
        """
    )
...
def save_workspace_to_db(app: web.Application, workspace: Workspace) -> None:
    app["db"].execute(
        """
        INSERT INTO workspaces (
            workspace_id, owner_id, name, status, base_directory, shell_path,
            home_root, project_root, idle_timeout_seconds, container_image,
            resource_profile, cpu_limit, memory_limit_mb, repo_url, branch,
            "commit", bootstrap_status, assigned_worker_id, created_at,
            last_activity_at, suspended_at, exit_code, last_error
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        (
            ...
            workspace.name,
            workspace.status,
            workspace.base_directory,
            workspace.shell_path,
            workspace.home_root,
            workspace.project_root,
            workspace.idle_timeout_seconds,
            workspace.container_image,
            workspace.resource_profile,
            workspace.cpu_limit,
            workspace.memory_limit_mb,
            workspace.repo_url,
            workspace.branch,
            workspace.commit,
            workspace.bootstrap_status,
...
def delete_workspace_from_db(app: web.Application, workspace_id: str) -> None:
    app["db"].execute("DELETE FROM workspaces WHERE workspace_id = ?", (workspace_id,))
    app["db"].commit()


def save_worker_to_db(app: web.Application, worker: WorkerRecord) -> None:
    app["db"].execute(
        """
        INSERT INTO workers (
            worker_id, url, status, registered_at, last_seen_at,
            active_usage_json, capacity_limits_json, last_error
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ON CONFLICT(worker_id) DO UPDATE SET
            ...
            active_usage_json=excluded.active_usage_json,
            capacity_limits_json=excluded.capacity_limits_json,
            last_error=excluded.last_error
        """,
        (
            worker.worker_id,
            ...
            worker.last_seen_at,
            json.dumps(worker.active_usage, sort_keys=True),
            json.dumps(worker.capacity_limits, sort_keys=True),
            worker.last_error,
        ),
    )
    app["db"].commit()
...
def load_control_plane_state(app: web.Application) -> None:
    WORKSPACES.clear()
    for row in app["db"].execute("SELECT * FROM workspaces WHERE status != 'deleted'"):
        workspace = workspace_from_row(row)
        WORKSPACES[workspace.workspace_id] = workspace

    registry = get_worker_registry(app)
    registry.clear()
    for row in app["db"].execute("SELECT * FROM workers"):
        worker = worker_from_row(row)
        registry[worker.worker_id] = worker


async def shutdown_control_plane_db(app: web.Application) -> None:
    db = app.get("db")
    if db is not None:
        db.close()
...
async def register_worker_handler(request: web.Request) -> web.Response:
    ...
    save_worker_to_db(request.app, record)
...
async def reconcile_worker_failures(app: web.Application) -> None:
    ...
    for workspace in list(WORKSPACES.values()):
        ...
        save_workspace_to_db(app, workspace)


async def fetch_worker_runtimes(session: aiohttp.ClientSession, worker: WorkerRecord) -> dict:
    try:
        async with session.get(f"{worker.url}/runtimes") as response:
            if response.status >= 400:
                return {"worker_id": worker.worker_id, "status": "error", "error": await response.text(), "runtimes": []}
            payload = await response.json()
            payload["status"] = "ok"
            return payload
    except aiohttp.ClientError as exc:
        return {"worker_id": worker.worker_id, "status": "error", "error": str(exc), "runtimes": []}


async def reconcile_runtime_state(app: web.Application) -> None:
    await refresh_workers_once(app)
    healthy_workers = get_healthy_workers(app)
    if not healthy_workers:
        for workspace in WORKSPACES.values():
            if workspace.status in {"ready", "starting", "cloning", "resuming", "suspending"}:
                workspace.mark_worker_lost("no healthy workers were reachable during control-plane recovery")
                save_workspace_to_db(app, workspace)
        return

    session = get_http_session(app)
    runtime_results = await asyncio.gather(*(fetch_worker_runtimes(session, worker) for worker in healthy_workers))
    observed: dict[str, tuple[str, dict]] = {}
    known_workspace_ids = set(WORKSPACES)
    ...
    for result in runtime_results:
        ...
        if result.get("status") != "ok":
            ...
            if worker is not None:
                worker.status = "degraded"
                worker.last_error = result.get("error", "runtime list failed")
                save_worker_to_db(app, worker)
            continue
        for runtime in result.get("runtimes", []):
            ...
            if workspace_id not in known_workspace_ids:
                print(f"[{utc_now_iso()}] orphan runtime {workspace_id} reported by {worker_id}; leaving it unmanaged")
                continue
            observed[workspace_id] = (worker_id, runtime)

    for workspace in WORKSPACES.values():
        workspace.websocket_attached = False
        observed_runtime = observed.get(workspace.workspace_id)
        if observed_runtime is not None:
            worker_id, runtime = observed_runtime
            workspace.assigned_worker_id = worker_id
            workspace.status = runtime.get("status", workspace.status)
            workspace.bootstrap_status = runtime.get("bootstrap_status", workspace.bootstrap_status)
            workspace.branch = runtime.get("branch", workspace.branch)
            workspace.commit = runtime.get("commit", workspace.commit)
            workspace.last_error = runtime.get("last_error")
            workspace.exit_code = runtime.get("exit_code")
            workspace.suspended_at = runtime.get("suspended_at")
            workspace.last_activity_at = runtime.get("last_activity_at", workspace.last_activity_at)
            save_workspace_to_db(app, workspace)
            continue

        if workspace.status in {"suspended", "error", "stopped"}:
            save_workspace_to_db(app, workspace)
            continue

        assigned_worker = get_worker_registry(app).get(workspace.assigned_worker_id or "")
        if assigned_worker is None or assigned_worker.status != "healthy":
            workspace.mark_worker_lost(
                f"assigned worker {workspace.assigned_worker_id or 'unknown'} was not healthy during control-plane recovery"
            )
        else:
            workspace.status = "suspended"
            workspace.suspended_at = utc_now_iso()
            workspace.last_error = "runtime was not reported by its healthy assigned worker during control-plane recovery"
...
async def create_workspace_handler(request: web.Request) -> web.Response:
    ...
    async with request.app["scheduler_lock"]:
        ...
        WORKSPACES[workspace.workspace_id] = workspace
        save_workspace_to_db(request.app, workspace)
        ...
        if status_code >= 400:
            ...
            workspace.last_error = worker.last_error
            save_worker_to_db(request.app, worker)
            save_workspace_to_db(request.app, workspace)
    ...
    async with request.app["scheduler_lock"]:
        ...
        if status_code == 404:
            ...
            if create_status < 400:
                ...
                return web.json_response(workspace.to_dict())
            workspace.status = "error"
            workspace.last_error = create_payload.get("error", "runtime recreate failed")
            save_workspace_to_db(request.app, workspace)
            return web.json_response(create_payload, status=create_status)
        workspace.last_error = payload.get("error", "runtime resume failed")
        save_workspace_to_db(request.app, workspace)
        return web.json_response(payload, status=status_code)
...
async def delete_workspace_handler(request: web.Request) -> web.Response:
    ...
    workspace.websocket_attached = False
    workspace.mark_activity()
    delete_workspace_from_db(request.app, workspace.workspace_id)
    return web.json_response(workspace.to_dict())
...
async def start_background_tasks(app: web.Application) -> None:
    await reconcile_runtime_state(app)
...
def main() -> None:
    ...
    app = web.Application()
    app["db"] = open_control_plane_db()
    app["workers"] = {}
    app["active_client_sockets"] = {}
    app["scheduler_lock"] = asyncio.Lock()
    load_control_plane_state(app)
    ...
    app.on_shutdown.append(shutdown_control_plane_db)

    print("Stage 13 durable control plane")
    print(f"Listening on http://{HOST}:{PORT}")
    print(f"SQLite state: {CONTROL_PLANE_DB_PATH}")
    print("Workers register dynamically via POST /workers/register")
    print("Recovery policy: load SQLite state, poll workers, query /runtimes, and reconcile workspace runtime status")
