...
SERVER_ROOT = "http://127.0.0.1:8016"
...
def print_workspace_summary(workspace_snapshot: dict, workspace_list_result: dict) -> None:
    print("")
    ...
    print(f"resource_profile: {workspace_snapshot['resource_profile']}")
    print(f"cpu_limit: {workspace_snapshot['cpu_limit']}")
    print(f"memory_limit_mb: {workspace_snapshot['memory_limit_mb']}")
    print(f"assigned_worker_id: {workspace_snapshot['assigned_worker_id']}")
    print(f"active workspaces for demo server: {len(workspace_list_result['workspaces'])}")
    usage = workspace_list_result["active_usage"]
    limits = workspace_list_result["capacity_limits"]
    owner_limits = workspace_list_result.get("owner_limits", {})
    print(
        "active usage: "
        f"{usage['workspace_count']} workspaces, "
        f"{usage['cpus']:.1f}/{limits['max_active_cpus']:.1f} CPUs, "
        f"{usage['memory_mb']}/{limits['max_active_memory_mb']} MB"
    )
    if owner_limits:
        print(
            "per-owner active limits: "
            f"{owner_limits['max_active_workspaces']} workspaces, "
            f"{owner_limits['max_active_cpus']:.1f} CPUs, "
            f"{owner_limits['max_active_memory_mb']} MB"
        )
    print("")
...
async def main() -> int:
    print("Durable control-plane workspace demo client")
    print("This client talks to a control plane that reloads workspace metadata and reconciles live worker runtimes after restart.")
    ...
    resource_profile = (
        await asyncio.to_thread(
            input,
            f"Resource profile [small|medium|large] (press Enter for {DEFAULT_RESOURCE_PROFILE}): ",
        )
    ).strip() or DEFAULT_RESOURCE_PROFILE
    ...
    async with aiohttp.ClientSession() as http_session:
        try:
            ...
            print("Start stage13_server.py and the Stage 13 workers in other terminals first.")
            return 1
        except RuntimeError as exc:
            print(exc)
            return 1
        ...
        try:
            if existing_workspace is None:
                ...
                if workspace["status"] in {"suspended", "worker_lost"}:
                    workspace = await resume_workspace(http_session, workspace["workspace_id"])
                    print(f"Resumed workspace {workspace['workspace_id']} from status {existing_workspace['status']}.")
        except RuntimeError as exc:
            print(exc)
            return 1
        ...
        workspace_list = await list_workspaces(http_session)
        print_workspace_summary(workspace_snapshot, workspace_list)
        ...
        try:
            while True:
                ...
                if ws is None or shell_closed.is_set():
                    print("Shell is not currently attached. Use /resume if the workspace is suspended or lost its worker.")
                    continue
