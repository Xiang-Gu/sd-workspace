    ...
    return web.json_response(
        {
            ...
            "entries": list_directory_entries(target_path, workspace.working_directory),
        }
    )
...
async def workspace_websocket_handler(request: web.Request) -> web.StreamResponse:
    ...
    try:
        async for msg in ws:
            ...
            print(
                f"[{utc_now_iso()}] input for workspace {workspace.workspace_id}: "
                f"{payload['data']!r}"
            )
    finally:
...
def main() -> None:
    ...
    print("Workspace plus filesystem demo server")
