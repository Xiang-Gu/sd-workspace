...
DATA_ROOT = os.path.join(BASE_DIRECTORY, ".stage11_data")
...
def get_container_name(worker_id: str, workspace_id: str) -> str:
    return f"stage11-{worker_id}-{workspace_id}"
...
async def choose_worker(app: web.Application, workspace: Workspace, preferred_worker_id: str | None = None) -> WorkerRecord:
    ...
    if preferred_worker_id:
        healthy.sort(key=lambda item: item.worker_id != preferred_worker_id)
    ...
    candidates.sort(key=lambda item: (item.active_usage["workspace_count"], item.active_usage["cpus"], item.worker_id))
    return candidates[0]
...
async def create_workspace_handler(request: web.Request) -> web.Response:
    ...
    try:
        ...
        return web.json_response({"error": str(exc)}, status=409 if "worker" in str(exc) or "healthy" in str(exc) else 400)
