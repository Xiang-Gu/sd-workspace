...
SERVER_ROOT = "http://127.0.0.1:8004"
...
async def main() -> int:
    print("Repo-aware workspace demo client")
    print("This client creates one workspace, optionally bootstraps a repo, and exposes file slash commands.")
    ...
    working_directory = (
        await asyncio.to_thread(
            input,
            "Workspace root (press Enter to use the current demo directory on the server): ",
        )
    ).strip()
    ...
    async with aiohttp.ClientSession() as http_session:
        try:
            ...
            print("Start stage5_server.py in another terminal first.")
            return 1
        except RuntimeError as exc:
            print(exc)
            return 1
        ...
        print(f"workspace root: {workspace_snapshot['workspace_root']}")
        print(f"project root: {workspace_snapshot['project_root']}")
